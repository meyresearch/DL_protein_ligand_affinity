...
#Setting the protein and ligand properties for feature extraction
...
def atom_features(atom):
    ...
    #One-hot encoding of the degree of the atom in the molecule, which is the number of directly-bonded neighbors (atoms)- 11
...
def smile_to_graph(smile,ligand_method,dataset):
    ...
    if ligand_method=="random_node":
        ...
        for atom in mol.GetAtoms():
            ...
            feature = np.random.choice(sample_arr, size=78)
            features.append(feature / sum(feature))
...
# contact map saved in data/dataset/cmap_dir
def target_to_graph(target_key, target_sequence, contact_dir, aln_dir):
    target_edge_index = []
    target_size = len(target_sequence)
    contact_file = os.path.join(contact_dir, target_key + '.npy')
    contact_map = np.load(contact_file)
    contact_map = np.add(np.matrix(np.eye(contact_map.shape[0])),contact_map,casting="unsafe")
...
def valid_target1(key, dataset,method):
    contact_dir = 'data/' + dataset +"/"+ method+"/"
...
def create_dataset_for_test(dataset, method,method1):
    # load dataset
    ...
    for pair_ind in range(len(rows)):
        ...
        if not valid_target1(prot_keys[cols[pair_ind]], dataset, method):
            continue
        ls = []
    ...
    for key in target_key:
        if not valid_target1(key, dataset, method):  # ensure the contact and aln files exists
            continue
...
def create_dataset_for_test_bootstrap(dataset, method,random,method1):
    # load dataset
    dataset_path = 'data/' + dataset + '/'
    test_fold = json.load(open(dataset_path + 'folds/test_fold_setting2.txt'))
    ##test_fold = json.load(open(dataset_path + 'folds/test_fold_setting1.txt'))
    ...
    test_drugs, test_prot_keys, test_Y = list(df_test['compound_iso_smiles']), list(df_test['target_key']), list(
        df_test['affinity'])
    test_drugs, test_prot_keys, test_Y = np.asarray(test_drugs), np.asarray(test_prot_keys), np.asarray(test_Y)
    test_drugs, test_prot_keys, test_Y=resample(test_drugs, test_prot_keys, test_Y,n_samples=1000,random_state=random)
    test_dataset = DTADataset(root='data', dataset=dataset + '_test', xd=test_drugs, y=test_Y,
                              target_key=test_prot_keys, smile_graph=smile_graph, target_graph=target_graph)
...
def create_dataset_for_train(dataset, fold, method,method1):
    # load dataset
    dataset_path = 'data/' + dataset + '/'
    ...
    for opt in opts:
        if opt == 'train':
            ...
            train_fold_entries = []
            for pair_ind in range(len(rows)):
                if not valid_target1(prot_keys[cols[pair_ind]], dataset,method):  # ensure the contact and aln files exists
                    continue
                ls = []
                ls += [drugs[rows[pair_ind]]]
                ls += [prots[cols[pair_ind]]]
    ...
    df_valid_fold = pd.read_csv('data/' + dataset + '_' +method + '_'+ 'fold_' + str(fold) + '_'  + 'valid' + '.csv')
